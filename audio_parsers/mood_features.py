...
def extract_mood_features(file_path: str, sr: int = 22050) -> list:
    """
    ...
    Возвращает:
        list: Нормализованный вектор признаков (1D)
    """

    # 1. Загрузка аудио --------------------------------------------------------
    # Загружаем первые 10 секунд аудио с частотой дискретизации sr как у аудио
    audio, _ = librosa.load(file_path, duration=10, sr=sr)
...
# audio_path = r"..\unbalanced_audio\audio1.wav"  # Замените на путь к вашему аудиофайлу
# features_vector = extract_mood_features(audio_path)
#
# if features_vector is not None:
#     print("Размер вектора признаков:", features_vector.shape)
# else:
